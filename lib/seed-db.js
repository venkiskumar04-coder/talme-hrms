...
export async function ensureSeedData() {
  ...
  if (employeeCount === 0 && employeeMasterSeed.length) {
    await prisma.employee.createMany({ data: employeeMasterSeed });
  }

  if (leaveCount === 0 && leaveSeed.length) {
    await prisma.leaveRequest.createMany({ data: leaveSeed });
  }

  if (attendanceCount === 0 && attendanceSeed.length) {
  ...
  if (documentCount === 0 && documentSeed.length) {
    await prisma.documentRecord.createMany({ data: documentSeed });
  }

  if (approvalCount === 0 && approvalSeed.length) {
  ...
  if (uploadedAssetCount === 0 && uploadedAssetSeed.length) {
